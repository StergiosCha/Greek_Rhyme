...
# -*- coding: utf-8 -*-
"""
Merge TXT and XLSX ENHANCED corpora (with full context)
"""

import json

print("=== Merging Complete Enhanced Corpus (TXT + XLSX) ===\n")

# Load TXT enhanced corpus
print("Loading TXT enhanced corpus...")
with open('unified_corpus_enhanced.json', 'r', encoding='utf-8') as f:
    txt_enhanced = json.load(f)

# Load XLSX enhanced corpus
print("Loading XLSX enhanced corpus...")
with open('rhyme_corpus_enhanced.json', 'r', encoding='utf-8') as f:
...
if 'entries' in xlsx_enhanced:
    for entry in xlsx_enhanced['entries']:
        entry['source'] = 'XLSX'
        # XLSX format is slightly different, normalize it
        if 'rhyme_pair' not in entry and 'lines' in entry:
            # Convert old format to new
            entry['rhyme_pair'] = entry.get('lines', [])
            entry['context'] = entry.get('lines', [])  # XLSX doesn't have full context, just the pair
        all_entries.append(entry)
    print(f"  Added {len(xlsx_enhanced['entries'])} XLSX entries")

# Create unified enhanced corpus
complete_enhanced = {
    "version": "complete_enhanced_v1",
    "description": "Complete Greek rhyme corpus with full context from TXT and XLSX sources",
...
# Save
with open('complete_corpus_enhanced.json', 'w', encoding='utf-8') as f:
    json.dump(complete_enhanced, f, ensure_ascii=False, indent=2)

print(f"\n✓ Saved complete_corpus_enhanced.json")
print(f"\n{'='*60}")
print(f"COMPLETE ENHANCED CORPUS (with context)")
print(f"{'='*60}")
