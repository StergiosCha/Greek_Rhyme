...
def detect_stress_position(word: str) -> List[Tuple[int, str]]:
    """
    Detect stress position in Greek word or phrase.
    Returns a LIST of possible classifications due to flexible syllabification.
    Each item is (syllable_from_end, classification).
    
    FIXED: Handles clitics correctly (e.g., 'όνομά της' -> F2, not F3)
    
    e.g. 'τέτοιος' -> [(2, 'F2'), (3, 'F3')] (if synizesis applies or not)
    """
    word = unicodedata.normalize('NFC', word)
    
    # CLITIC HANDLING: If phrase has unstressed final word, adjust stress position
    words = word.split()
    if len(words) > 1:
        last_word = words[-1]
        # Check if last word has no accent (clitic)
        accent_chars = 'άέήίόύώΐΰ'
        if not any(c in accent_chars for c in last_word):
            # Last word is clitic (e.g., 'της', 'μου', 'σου') OR stressed monosyllable (e.g. 'φως')
            # The stress position shifts ONLY if the previous word is accented.
            # e.g. 'σώμα μου' -> 'σώμα' has accent -> Shift -> F2
            # e.g. 'το φως' -> 'το' has NO accent -> No Shift -> M
            
            accented_word = words[-2] if len(words) >= 2 else words[0]
            
            # Check if previous word has accent
            prev_has_accent = any(c in accent_chars for c in accented_word)
            
            if prev_has_accent:
                base_stress = detect_stress_position(accented_word)
                
                # Shift stress position by 1 (add clitic syllable)
                shifted_results = []
                for pos, stype in base_stress:
                    new_pos = pos + 1
                    if new_pos == 2:
                        shifted_results.append((2, 'F2'))
                    elif new_pos == 3:
                        shifted_results.append((3, 'F3'))
                    else:
                        shifted_results.append((new_pos, 'F3'))  # F3+ for rare cases
                return shifted_results if shifted_results else base_stress
            else:
                # Previous word has no accent (e.g. 'το', 'στη', 'και')
                # So the last word MUST be the stressed one (e.g. 'φως', 'νους', 'γη')
                # Since it's a monosyllable (otherwise it would have accent), it's M.
                return [(1, 'M')]
    ...
    # IMPORTANT: Use the LAST accent if multiple (e.g., 'όνομά' has stress on 'ά', not 'ό')
    stress_pos = -1
    for i, char in enumerate(word):
        if char in accent_chars:
            stress_pos = i  # Keep updating to get the LAST accent
...
def extract_rhyme_domain(line: str, include_context: bool = False) -> Dict:
    ...
    """
    # Normalize Unicode (NFC) to ensure accents are combined characters
    ...
    if not has_accent and len(words) >= 2:
        ...
        if any(c in 'άέήίόύώΐΰ' for c in prev_word):
            ...
            # Prev word has no accent.
            # Check for Monosyllable + Clitic case (e.g. "βρει σου")
            if count_syllables(prev_word) == 1:
                 rhyme_words = [prev_word, last_word]
                 rhyme_domain = f"{prev_word} {last_word}"
                 stress_type = 'F2'
            else:
                 # Both unstressed and prev not monosyllable?
                 pass
...
def classify_rhyme_pair(word1: str, word2: str) -> Dict:
    ...
    # Check IMP-C (Vowels Match, Consonants Differ)
    if vs1 == vs2:
        # STRICTNESS CHECK: Open vs Closed Syllable Mismatch
        # In Standard Rhyme, we reject mismatches between Open (ends in V) and Closed (ends in C) syllables.
        # e.g. 'gelane' (Open) vs 'gennanes' (Closed) -> REJECT.
        is_open1 = pat1[-1] == 'V'
        is_open2 = pat2[-1] == 'V'
        
        if is_open1 != is_open2:
            return {'type': 'NONE', 'reason': 'Open/Closed syllable mismatch'}
        ...
        if cs1 != cs2:
            ...
            def are_consonants_compatible(c_list1, c_list2):
                ...
                if len(c_list1) != len(c_list2):
                     # Check subsequence
                     if len(c_list1) < len(c_list2):
                         # If c_list1 is empty (open syllable) and c_list2 is not (closed), REJECT for standard IMP-0
                         # unless we are in Topintzi mode (which this function doesn't know about directly, 
                         # but the caller 'classify_rhyme_pair' is for STANDARD/ENHANCED).
                         # Topintzi logic is in 'classify_rhyme_pair_topintzi'.
                         
                         # So for STANDARD, we should REJECT if one is open and other is closed?
                         # The user says "gelane" (open) vs "gennanes" (closed) is WRONG.
                         # So we must reject it here.
                         
                         # Check if one is empty (Open Syllable)
                         if not c_list1: return False # Reject Open vs Closed
                         
                         if is_subsequence(c_list1, c_list2): return "IMP-0"
                     else:
                         if not c_list2: return False # Reject Open vs Closed
...
def extract_pre_rhyme_vowel(w: str) -> str:
    ...
    start_scan = accent_idx - 1
    if start_scan >= 0:
        prev = w_norm[start_scan].lower()
        curr = w_norm[accent_idx].lower()
        is_digraph = False
        if prev == 'ο' and curr in ['ύ', 'ί']: is_digraph = True
        elif prev == 'ε' and curr == 'ί': is_digraph = True
        elif prev == 'α' and curr == 'ί': is_digraph = True
        elif prev == 'υ' and curr == 'ί': is_digraph = True
        
        if is_digraph:
            start_scan -= 1

    # Handle Synizesis: If char before accent (or digraph) is 'ι', it's likely a glide
    end_idx = start_scan + 1 # end_idx is exclusive for slicing, so +1 to include start_scan?
    # Wait, slicing w_norm[:end_idx].
    # If start_scan is the index of the last char we want to KEEP?
    # No, start_scan is where we start scanning BACKWARDS for the vowel.
    # But here we want to return `pre_chunk`.
    # `pre_chunk` is used by `g2p_greek`.
    # If we exclude the digraph part, `g2p` might be confused if we cut in middle?
    # Actually, `get_pre_stress_vowel` logic is:
    # 1. Get pre_chunk.
    # 2. Phoneticize pre_chunk.
    # 3. Find last vowel.
    
    # If we cut off the 'ο' of 'ού', pre_chunk ends before 'ο'.
    # So `g2p` sees ...ματ. Last vowel is 'α'. Correct.
    
    # So we just need to set `end_idx` correctly.
    # If no digraph, end_idx = accent_idx. (Slicing excludes accent_idx).
    # If digraph, end_idx = accent_idx - 1. (Slicing excludes accent_idx-1).
    
    end_idx = accent_idx
    if start_scan == accent_idx - 2: # Digraph detected
        end_idx = accent_idx - 1
        
    # Check for synizesis at the new end
    # If w_norm[end_idx-1] is 'ι', skip it?
    if end_idx > 0 and w_norm[end_idx-1] == 'ι':
        end_idx -= 1
        
    # Handle diphthongs 'αυ', 'ευ' where 'υ' is consonant [f/v]
    # If char is 'υ', check if previous char is 'α' or 'ε'.
    if end_idx > 0 and w_norm[end_idx-1] == 'υ':
        if end_idx > 1 and w_norm[end_idx-2] in ['α', 'ε']:
            end_idx -= 1
            
    pre_chunk = w_norm[:end_idx]
